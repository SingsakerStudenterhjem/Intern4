...
const RegiLogs: React.FC = () => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="max-h-[70vh] overflow-auto border border-gray-200 rounded-sm bg-white shadow-sm">
            ...
            {!loading &&
              filtered.map((log) => (
                <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-2 text-sm text-gray-700">{log.approvedByName ?? '—'}</td>
                  <td className="p-2 text-sm">
                    {log.comment && <div className="text-xs text-gray-500 mt-1">{log.comment}</div>}
                  </td>
                </tr>
              ))}
